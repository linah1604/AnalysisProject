...
- The struct definitions and variable names in `inputs.hpp` were human-written by the students.
...
### Dataset (`data/`)
- `workers.csv`, `orders.csv`, and `order_items.csv` were designed by the students to reflect realistic warehouse scenarios though data was generated by AI and to cover the specific test cases documented in `docs/test_cases.md`.
